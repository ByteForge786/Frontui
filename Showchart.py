...
# Generate automatic chart based on data
def generate_chart(df):
    try:
        # If the dataframe has numeric and categorical columns, create a box plot
        numeric_cols = df.select_dtypes(include=[np.number]).columns
        categorical_cols = df.select_dtypes(include=['object', 'category']).columns
        
        if len(numeric_cols) > 0 and len(categorical_cols) > 0:
            fig = px.box(df, x=categorical_cols[0], y=numeric_cols[0], title=f"{numeric_cols[0]} by {categorical_cols[0]}")
        # If only numeric columns, create a histogram
        elif len(numeric_cols) > 0:
            fig = px.histogram(df, x=numeric_cols[0], title=f"Distribution of {numeric_cols[0]}")
        # If only categorical columns, create a bar chart
        elif len(categorical_cols) > 0:
            value_counts = df[categorical_cols[0]].value_counts()
            fig = px.bar(x=value_counts.index, y=value_counts.values, title=f"Count of {categorical_cols[0]}")
        else:
            return None
...
def main():
    ...
    with right_column.container():
        ...
        with st.chat_message(name="assistant", avatar="assistant"):
            ...
            chart_button_placeholder = st.empty()
            ...
            if st.button("🚀 Generate SQL", key="generate_sql", use_container_width=True):
                if user_input:
                    ...
                    try:
                        ...
                        chart_button_placeholder.button("📊 Show/Hide Chart", on_click=toggle_chart, use_container_width=True)
                        
                        # Display chart if show_chart is True
                        if st.session_state['show_chart']:
                            fig = generate_chart(result_df.head(1000))  # Limit to 1000 rows for performance
                            if fig:
                                chart_placeholder.plotly_chart(fig, use_container_width=True)
                            else:
                                chart_placeholder.warning("Could not generate chart for this data structure")
        ...
        # Sample questions section remains the same...
